...
# Configure PHP
ARG PHP_EXTENSIONS "bcmath intl opcache pcntl redis zip"
COPY --link --from=extension-installer /usr/bin/install-php-extensions /usr/local/bin/
RUN set -eux; \
...
ARG PHP_DEV_EXTENSIONS "pcov xdebug"
RUN set -eux; \
    install-php-extensions $PHP_DEV_EXTENSIONS; \
...
COPY --link --chown:ww-data:www-data . .
...
COPY --link --chown:ww-data:www-data . .
RUN composer dump-autoload --classmap-authoritative --no-dev --strict-psr
